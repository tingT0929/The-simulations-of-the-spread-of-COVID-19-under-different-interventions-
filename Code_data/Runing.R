setwd("D:/Desktop/Research/Paper/Data-driven analysis on the simulations of the spread of COVID-19 under different interventions of China/Code/")
...
init_gene <- function(it){
  ...
  dpa <- runif(1, 1, 20)
  return(list(para, alp, dpa, -Inf))
}

l <- function(para_init, N, time_length, dat, init){
  return(-likelihood(init, c(1, 2, 5.1, 9.5), 
...
# Burn-in 
a <- sapply(1:K, function(i){Result[[i]][[4]]})
...
for(Cpu in 1:100){
  ...
  a <- sample(1:K, replace = T, prob = exp(a - max(a)))
  para_t <- lapply(a, function(i){Result[[i]]})
  cat(c(max(sapply(1:K, function(i){Result[[i]][[4]]}))), "\n")
}
...
pred <- function(k, time_length, para, init, N){
  comp_num <- Dynamic(time_length, para[[k]][[1]], para[[k]][[2]], init, N)
  colnames(comp_num) <- c("S", "E", "I", "C", "H")
  return(comp_num)
}

Result <- lapply(1:length(para), pred, para = para, time_length = time_length, init = init, N = N)

pred_array <- array(0, c(length(para), time_length, 5))
...
Rt <- sapply(1:length(para), function(k){
  ...
  return(rt / (1 / para[[k]][[1]][3] + 1 / para[[k]][[1]][4]))
})
Rt_mean <- rowMeans(Rt)
